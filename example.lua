...
AddEventHandler("onDatabaseConnect", function (databaseName)
    print("[MongoDB][Voorbeeld] onDatabaseConnect: "..tostring(databaseName))
    ...
    exports.mongodb:findOne({ collection="users", query = { username = username } }, function (success, result)
        if not success then
            print("[MongoDB][Voorbeeld] Error in findOne: "..tostring(result))
            return
        end
        -- Print user if already exists
        if #result > 0 then
            print("[MongoDB][Voorbeeld] User is already created")
            printUser(result[1]._id)
            exports.mongodb:updateOne({ collection="users", query = { _id = result[1]._id }, update = { ["$set"] = { first_name = "Bob" } } })
            return
        end
        print("[MongoDB][Voorbeeld] User does not exist. Creating...")
        exports.mongodb:insertOne({ collection="users", document = { username = username, password = "123" } }, function (success, result, insertedIds)
            if not success then
                print("[MongoDB][Voorbeeld] Error in insertOne: "..tostring(result))
                return
            end
            print("[MongoDB][Voorbeeld] User created. New ID: "..tostring(insertedIds[1]))
    ...
    exports.mongodb:count({collection="users"}, function (success, result)
        if not success then
            print("[MongoDB][Voorbeeld] Error in count: "..tostring(result))
            return
        end
        print("[MongoDB][Voorbeeld] Current users count: "..tostring(result))
        if result < 10 then
            local insertUsers = {}
            for i = 1, 10 do
            ...
            exports.mongodb:insert({ collection="users", documents = insertUsers }, function (success, result)
                if not success then
                    print("[MongoDB][Voorbeeld] Failed to insert users: "..tostring(result))
                    return
                end
                print("[MongoDB][Voorbeeld] Inserted "..tostring(result).." new users")
